...
format with `mlx_lm.convert -q --q-bits 4` (4.5 bits per weight, 2.4 GB). It is the small MLX version, about half the size of the
[8-bit version](https://huggingface.co/peterbuitho/VietPoet-Qwen3.5-4B-MLX-8bit), which is the near-lossless one. For other
...
held-out "8 câu" prompts served by `mlx_lm.server` on an Apple M2 Pro: rule score **0.995**, **100%** of poems fully
valid (the 8-bit version on the same prompts: 0.992 and 95%, the 16-bit model on vLLM: 0.994 and 98% on 100 prompts;
with 40 prompts a difference of one or two poems is noise). Without the sampler the raw model scores about 0.81 and
...
```bash
pip install mlx-lm
mlx_lm.server --model peterbuitho/VietPoet-Qwen3.5-4B-MLX-4bit --port 8080 --prompt-cache-size 0
```
